...
Considering that this is a process that is recurrent in my compositional practice and I always sketch the existing scores in similar fashion on paper, I decided to programme an application in SuperCollider that takes information from a MIDI file and creates a visualization of its phrase structure. Therefore, I developed extensions for the SimpleMIDIFile\footnote{SimpleMIDIFile accesses MIDI file information in SuperCollider. See footnote number \hyperlink{wlib}{14}.} class to perform these operations. The message \emph{trackSilence}\footnote{This extension can be found as an extension of SimpleMIDIFile at URL: } analyses a MIDI file track to locate the starting and ending points of silences. This is done by analyzing the MIDI note-on and note-off messages.
