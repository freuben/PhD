...
\end{verbatim}
Ones the results for the silences of each track of the MIDI file are obtained, it is possible to create a 
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=8cm]{Chapter4/midi_phrase.tif} %change centimeters for resizeing
   \caption{Visualization of Ockeghem's \emph{Missa Mi mi}.}
   \label{fig:example}
\end{figure}\
Each MIDI track is represented as a different color and the silences are displayed in light grey. The x-axis represents time, where each square equals a value for time that can be specified by the user, thereby making it possible for the user to `zoom' in and out of the phrase structure.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=16cm]{Chapter4/midi_gesualdo.tif} %change centimeters for resizeing
